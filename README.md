...
使用 NovelAI API Proxy 时,在引导页的
「第三方」或「我的 → 令牌管理 → 添加令牌 → 第三方」中选择 `API Proxy`,填写代理根地址
和管理员签发的**客户端 key**。不要填写管理面板地址或官方 Token。保存时 App 会调用
`/quota` 验证 key,令牌管理页展示该 key 的本地剩余额度、待核对量与队列状态;
出图、V5 扩散超分和 Vibe 编码继续使用原有 NovelAI 图片协议。生产环境应使用 HTTPS。
代理不提供账号密码登录或传统超分,额度是代理为该 key 分配的预算,并非官方账目。
